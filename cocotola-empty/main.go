...
import (
	...
	mblibdomain "github.com/mocoarow/cocotola-1.25/moonbeam/lib/domain"
...
func run() (int, error) {
	...
	defer shutdownlog()
	logger := slog.Default().With(slog.String(mblibdomain.LoggerNameKey, AppName+"-main"))

	// init tracer
	shutdownTrace, err := mblibconfig.InitTracerProvider(ctx, cfg.Trace, AppName)
	if err != nil {
